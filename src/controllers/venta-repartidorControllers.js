...
const registrarVenta = async (req, res) => {
    const { repartidor_id, total, productos, foto_venta } = req.body; // Se espera un array de productos

    // Verificar que todos los campos necesarios están presentes
    if (!repartidor_id || !total || !foto_venta || !productos || productos.length === 0) {
    ...
    try {
        ...
        const queryVenta = "INSERT INTO ventas_repartidores (repartidor_id, total, foto_venta) VALUES (?, ?, ?)";
        const [ventaResult] = await connection.query(queryVenta, [repartidor_id, total, foto_venta]);
